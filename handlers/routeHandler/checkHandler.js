...
handler._check.post = (requestProperties, callback) => {
    ...
    const timeOutSeconds =
        typeof requestProperties.body.timeOutSeconds === 'number' &&
            requestProperties.body.timeOutSeconds && requestProperties.body.timeOutSeconds >= 1 && requestProperties.body.timeOutSeconds <= 5 ? requestProperties.body.timeOutSeconds : false;
                            ...
                            if (tokenIsValid) {
                                ...
                                if (userChecks.length < maxChecks) {
                                    ...
                                    data.create('checks', checkId, checkObject, (err3) => {
                                        if (!err3) {
                                            // add checkId in the user object
...
handler._check.put = (requestProperties, callback) => {
    ...
    if (id) {
        if (protocol || url || method || successCodes || timeOutSeconds) {
            data.read('checks', id, (err1, checkedData) => {
                if (!err1 && checkedData) {
                    let checkObject = parseJSON(checkedData);
                    let token = typeof requestProperties.headersObject.token === 'string' ? requestProperties.headersObject.token : false;
